...
$dockerTag  = 'tc-dev-visionwheel-shop-copy'; # docker image to deploy
# $dockerTag  = 'dev-shop'; # docker image to deploy
$dockerService = 'tc-dev-visionwheel-shop-copy'; # service to deploy as

# AWS ECS cluster, service and task info

$environment  = 'production';           # server environment type ('qa' or 'production')
...
# $taskName     = "dev-shop";             # name for task service will provide

$deployURL    = "https://vw-dev-api.visionwheel.com/";
...
runCmd( "$dockerCmd build -f site/Dockerfile.tc.prod --memory='4g' --memory-swap='4g' -t $dockerTag ." , $verbose );
