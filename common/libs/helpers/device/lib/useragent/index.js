 ...
 */
var LRU = require('../../../cache/lru-cache')(5000),
	regexps = require('./lib/regexps'),
	updater = require('./lib/update');
	...
	get: function lazyparse() {
		...
		return Object.defineProperty(this, 'os', {
			value: !parser || !res ?
				new OperatingSystem() :
				new OperatingSystem(
					res[1], parser[2] || res[2], parser[3] || res[3], parser[4] || res[4]
				)
		}).os;
	},
...
module.exports = function updater() {
	try {
		updater.update(function updating(err, results) {
			if (err) {
	...
	for (; i < length; i++) {
		if (res = parsers[i][0].exec(userAgent)) {
			...
			if (!jsAgent) return new Agent(
				res[1], parser[2] || res[2], parser[3] || res[3], parser[4] || res[4], userAgent
			);
