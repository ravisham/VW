...
$dockerTag  = 'visionwheel-admin-site'; # docker image to deploy
...
$service      = $dockerTag;	# service to deploy as (MUST ALREADY EXIST)
$taskName     = $dockerTag;         	# name for task service will provide

$deployURL    = "https://shop.visionwheel.com:8443";
...
runCmd( "$dockerCmd build -f admin/Dockerfile -t $dockerTag ." , $verbose );
